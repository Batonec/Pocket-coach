...
- [ios](./ios) — нативный iOS-клиент (SwiftUI) + [бриф карточки «Совет тренера»](./ios/RECOMMENDATION_CARD_BRIEF.md) + [бриф голосовой записи подхода](./ios/VOICE_LOGGING_BRIEF.md)
...
- записывать подход голосом через Siri, не доставая телефон из кармана: «Зал жим ногами» засчитывает подход по плану одной фразой, «Зал подход» → «жим ногами 80 на 10, тяжело» пишет произвольный (по-английски — через кодовое слово `Gym`), плюс отмена, вопрос «что дальше» и завершение тренировки голосом;
...
- Load-type (`heavy / medium / light`) хранится из применённого плана тренера (без плана — не задан) и в текущем UX на карточках истории не показывается.
