...
- «неделя закрыта» — с недельным отчётом тренера, который пишется заранее по воскресеньям.
...
**Backend — чистый stdlib Python 3.13**: ни зависимостей, ни venv, ни фреймворка; Claude API вызывается через `urllib`. Тестов — **250 на backend и 129 на iOS**, и это единственный автоматический гейт перед деплоем.
