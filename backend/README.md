...
с нулём эффективных сетов за 10+ дней обязана попасть в план), `rest_days` 0–4, «после
heavy не heavy». Нарушения уходят модели одним авто-репромптом; повторный промах —
ошибка генерации с деталями. Замеры талии хранятся в таблице `waists` (пишутся через
