...
- [backend/trainer/domain/plan_validator.py](./trainer/domain/plan_validator.py) — факты истории (`Bounds`) и словарь слов, которыми написаны правила плана; сами правила — текст в [prompts/plan_rules.md](./prompts/plan_rules.md), исполняет их `rule_engine`; санитизация ответа — в `rules.py`
- [backend/trainer/data/rule_engine.py](./trainer/data/rule_engine.py) — правила как текст: разбор блоков «для / требует / нарушение / починить» из markdown, выражения без `eval`, общий ход «проверить → назвать → починить»; про тренировки не знает ничего (см. «Правила плана как текст»)
...
- `GET /api/measurements` · `POST /api/measurements` · `DELETE /api/measurements/{id}` —
  обхваты кроме талии (`kind` из `limits.MEASUREMENT_KINDS`: рука расслабленно и в напряжении,
  предплечье, плечи, грудь, шея, бедро, ягодицы; `value_cm`), один замер вида в день. Совет не
  пересобирают: план обхваты не читает, их читает недельный отчёт. iOS-клиент их пока не пишет —
  штатный ввод через Coach MCP
...
тренерскую политику (волны интенсивности, RIR, прогрессия, deload, матрица питания)
и срез стратегии атлета; отдельного описания фаз в системном промпте нет — ориентиры
текущей фазы приходят первой строкой user-промпта из `phase_params`, без машинного имени. Политика подана явно как **ориентиры по
...
веса от пика; гравитрон инвертирован — прогресс там это уменьшение противовеса), **объём за
круг** из четырёх последних тренировок с целями по группам, прямыми и **эффективными** сетами
(жимы добирают трицепсу/дельтам, тяги — бицепсу; программа написана под четыре тренировки, атлет
ходит через день, поэтому план календарную неделю не судит вовсе — блок за 7 дней остался у
отчёта как темп),
**явку по календарным неделям** (пн–вс, последние четыре закрытые плюс текущая, серия
подряд с ≥3, темп за 14 дней и интервалы между последними сессиями), **прошлую карточку**
из кэша (фокус, состав, пункт «Совет» — обещание следующей сессии), флаг **застоя** по
**активному окну блока** — от старта фазы или первой
тренировки после перерыва ≥14 дней, не длиннее 6 недель, так что отпуск не размывает частоту:
предусловия — частота ≥2.5/нед, объём крупных групп за круг не ниже нижней границы `group_targets`
...
стека из истории) и ветку **матрицы питания**: тренд веса — МНК-наклон по всем замерам окна,
ветки — от коридора `rate_kg_per_week` фазы, а не от её имени, полоса ±0.12 кг/нед и шаг
калорий по режиму (150 на срезе, 75 на наборе, 100 на удержании) — контур коррекции из главы
«Питание» стратегии; отклонение становится советом только при подтверждении средними окна
(две недели на срезе и удержании, четыре на наборе). В сводке по
...
предложить смену в rationale. Цель по весу (`target_weight_kg`) и потолок набора
(`ceiling_weight_kg`) дефолтов не имеют — это цифры стратегии атлета, задаются в
`phase_params`; без них отчёт и iOS цель не показывают, матрица «цель достигнута» не
объявляет. Недельный объём строительных фаз идёт ramp'ом 6–8 →
...
(≥2 сессии/нед в блоке) сервер помечает 7-ю неделю разгрузочной (объём и веса лёгкой недели
задаёт стратегия, промпт чисел не повторяет), после неё ramp начинается заново; при редких
тренировках флаг не ставится.
...
**Смена этапа — ручной чек-лист.** Этап стратегии (Ф0, Ф1, …) — не фаза машины: `cut_recomp`
покрывает и Ф0, и Ф1, различаются они параметрами. Поэтому этап записан руками в трёх местах,
и при переходе меняются все три в один день:

1. блок «Цель» профиля (`coach_update_profile`): абзацы «ТЕКУЩИЙ ЭТАП» и «СЛЕДУЮЩИЙ ЭТАП» с
   гейтом выхода;
2. `phase_params` текущей фазы в `coach_state.json`: `title`, `calories`, `rate_text` и
   `rate_kg_per_week`, `session_sets` (для Ф1 не ниже 24 — Д4 каркаса это 24 подхода, иначе
   валидатор режет дни программы), `deload_every_weeks` (3 — микроцикл стратегии «база → база →
   база → разгрузка»), `group_targets`. Файл правится руками: `coach_set_phase` на ту же фазу
   сбросит дату старта. Развилка про дату: оставить `phase_started` — недели блока и ramp
   продолжаются, но матрица питания и калибровка TDEE читают замеры с начала Ф0; сбросить через
   `coach_set_phase` — замеры Ф0 отсекаются, но неделя блока и ramp объёма начнутся с начала;
3. `prompt_builder.STRATEGY_SECTIONS` и `REPORT_STRATEGY_SECTIONS`, если у нового этапа своя
   глава в стратегии (проверка — `coach_preview_prompt`: ненайденный заголовок печатается
   предупреждением).

По ходу Ф1 недели поддержки отмечаются `coach_mark_support_week`.
...
генерация больше не падает. Правила — текст в `prompts/plan_rules.md` (см. «Правила плана как
текст» ниже): формулировка блока рендерится в «ЖЁСТКИЕ ГРАНИЦЫ» системного промпта, остальные
строки того же блока исполняет сервер. Замеры талии хранятся в таблице `waists` (пишутся через
...
проигнорировано». Обхваты кроме талии (таблица `measurements`, вид + сантиметры, общие
границы 10–200 см из `limits`) — метрики цели из vision: их видит только недельный отчёт,
строкой «последний замер, дней с него, предыдущий» по каждому виду; периодичность протокола
модель берёт из главы «Измерения» стратегии, сервер её не судит. Правило свежести общее с весом: данные старше 14 дней → советы по
...
## Правила плана как текст

Жёсткие границы плана записаны в [prompts/plan_rules.md](./prompts/plan_rules.md) блоками
`## имя`. Блок — формулировка (абзац, который модель читает в системном промпте) и четыре ключа,
которые исполняет сервер:

```
для каждого: подход с возвратным потолком
требует: тяжесть <= потолок
нарушение: {{упражнение}}: {{что}} {{вес}} кг тяжелее доперерывного рабочего ({{допустимо}}) — …
починить: вес := допустимо
```

- `для` (`для каждой`, `для каждого`) — область: по чему идти. Фраза из словаря
  `plan_validator.SCOPES`; область объявляет имена, известные в каждом случае (для выражений и
  слотов), и те из них, которым можно присвоить.
- `требует` — условие, которое обязано быть истинным в каждом случае: сравнения (и цепочкой) и
  `and` / `or` / `not` над именами области и числами. Вызовов, атрибутов и арифметики нет:
  выражение читается через `ast` по короткому списку узлов, без `eval`.
- `нарушение` — строка для репромпта модели, слоты `{{имя}}` из области.
- `починить` — что сервер делает после второго промаха модели: `имя := выражение` или фраза
  процедуры из `plan_validator.PROCEDURES`. Без этой строки нарушение уходит пометкой в
  rationale. Тексты пометок для атлета — [resources/plan_notes.md](./resources/plan_notes.md):
  `имя_change` на одну правку, `имя_fixed` — заголовок; `fixed` и `unresolved` общие.

Читает и исполняет блоки `data/rule_engine.py`. Слово не из словаря, имя не из области,
выражение вне грамматики, слот без значения, процедура без словаря или пометка без починки —
`RuleError` при импорте, то есть на старте сервера. Новое правило — новый блок; новое слово в
словаре нужно, только если правило говорит о факте, которого сервер ещё не считает.
...
целиком и вместе с ними все юниты и таймеры из `infra/deploy/`. Каталоги синхронизируются с
удалением (`rsync --delete`, и в CI, и в скрипте): файл, которого в репозитории больше нет,
исчезает и с VPS; сиротские `.pyc` в `__pycache__` CI выметает перед рестартом. Таймеры деплой
не включает: новый таймер один раз `systemctl enable --now` руками.
