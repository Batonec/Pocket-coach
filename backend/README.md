...
**Возврат после перерыва** (`coach_features.return_ceilings`): при паузе ≥14 дней каждое
упражнение с историей получает потолок веса на первую сессию от собственного
доперерывного рабочего веса (2–3 нед ~90%, 3–5 нед ~85%, 5–8 нед ~80%, дольше ~75%,
с округлением к плитке тренажёра; гравитрон инвертирован). В промпт уходят и потолки, и
причина (детренированность связок/выносливости опережает потерю нервной силы), и
следствие: двойная прогрессия на возвратной сессии не применяется, отметки «легко»/RIR
описывают форму до перерыва. Валидатор enforce'ит потолки для упражнений, у которых нет
лестницы разгона (они стояли на пике) — иначе общий коридор ±15% пропускал прибавку веса
в первую же сессию после паузы.
...
Превышение верхней границы подходов сервер детерминированно срезает до коридора,
после чего снова проверяет весь план; это ограничение не может завалить генерацию.
Остальные нарушения уходят модели одним авто-репромптом; повторный промах — ошибка
генерации с деталями. Замеры талии хранятся в таблице `waists` (пишутся через
