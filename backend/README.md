...
10–14 возврат), `rest_days` 0–4, «после heavy не heavy». Нарушения уходят модели одним
авто-репромптом; повторный промах — ошибка генерации с деталями. Замеры талии хранятся в
таблице `waists` (пишутся через Coach MCP), правило свежести общее с весом: данные старше
14 дней → советы по калориям не даются, модель просит замер.
