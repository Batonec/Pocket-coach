...
Zero third-party dependencies on purpose: the HTTP call to api.anthropic.com
lives in ``anthropic_client`` on stdlib ``urllib``, so the long-running server
process gains no extra packages and no extra RAM footprint on the small VPS.
