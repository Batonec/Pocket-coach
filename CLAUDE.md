...
SQLite: workouts, body_weights, waists, events + coach_state.json + coach_profile.json
   ...
   │                    снапшот совета, даты) и решения при записи: что сохранить при ретрае и правке,
   ...
   │
   ├─ plan_validator.py Санитизация ответа по границам, которых нет в JSON-схеме, и три
   │                    жёсткие границы методики (см. ниже); при повторном промахе
   │                    модели — детерминированное разрешение с пометкой в rationale.
   │
...
**Валидатор проверяет ровно три жёсткие границы** (`plan_validator._semantic_violations`): покрытие групп
