...
SQLite: workouts, body_weights, waists, events + coach_state.json + coach_profile.json
   ...
   │                    plan_rules (формулировки жёстких границ и строки нарушений:
   │                    в системный промпт и в репромпт из одного файла).
   ...
   │
   ├─ plan_validator.py Три жёсткие границы методики таблицей RULES (имя, check, fix):
   │                    Bounds — факты истории, посчитанные до вызова модели;
   │                    violations — строки для репромпта; resolve — при повторном
   │                    промахе чинит, что можно, и дописывает пометку в rationale.
   │                    Формулировки — prompts/plan_rules.md под теми же именами.
...
**Валидатор проверяет ровно три жёсткие границы** (`plan_validator.RULES`): покрытие групп
...
Формулировки правил — `prompts/plan_rules.md`: оттуда они рендерятся в блок «ЖЁСТКИЕ ГРАНИЦЫ»
системного промпта и оттуда же берутся строки нарушений для репромпта, так что модель читает
ровно то, что сервер проверяет. `RulesTableTests` держит имена `RULES` и фрагменты один к
одному: правило без текста и текст без правила падают в CI, а не молчат в промпте.
...
только `[a-z_]+`: `{{streak3}}` с цифрой регулярка не увидит, и сборка упадёт как «лишний слот»
