...
#   3) тронута граница «алгоритм / LLM» (recommender.py, plan_validator.py,
#      anthropic_client.py, coach_signals.py).
set -uo pipefail
...
printf '%s\n' "$paths" \
  | grep -qE '^backend/(recommender|anthropic_client|plan_validator|coach_signals)\.py$' \
